# Elektro — Configuration Reference

This document explains how the **elektro** service is configured, then lists every
...
[`elektro_server/configuration.py`](elektro_server/configuration.py); this file documents
...
```bash
ARKITEKT_CONFIG_FILE=/etc/elektro/config.yaml python manage.py runserver
```
...
| `datalayer.protocol` | `DATALAYER__PROTOCOL` |

Lists and nested objects (e.g. `authentikate.issuers` or `datalayer.media`) are awkward
to express as environment variables — prefer the YAML file for those and use env vars for
...
top-level `s3` block). Unlike the optional datalayer in some services, this block is
**required** for elektro, and so are the `media` and `zarr` bucket bindings.
...
A bucket binding may also set `default_max_bytes` (bytes, default `100MiB`): the per-upload
budget advertised on grants when no quota sets `max_upload_bytes`.
...
Array, table and sparse datasets embed their name + description into a pgvector
...
The `search` argument of `arrayDatasets`, `tableDatasets` and `sparseDatasets` then matches
a substring of the name **or** a description whose
...
(`elektro_server/asgi.py` starts it; `embeddings/healer.py` is the loop) in row-locked batches —
...
migration that first nulls the three columns (`UPDATE core_arraydataset SET embedding = NULL,
embedding_model = ''`, likewise `core_tabledataset` and `core_sparsedataset` — Postgres refuses
...
Folders and experiments keep their own relevance-ranked search (full text plus trigram);
the semantic leg is not on them yet.
