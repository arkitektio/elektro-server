...
plain input. Export names a container from ``fromFileLike``, and there are two kinds of
container (mikro, which this is vendored from, has four), so it follows the flat-discriminated-union convention of ``core.input_unions``
...
_SERIES_DESCRIPTION = (
    "Which part of the file this concerns -- the series of a multi-series ABF or NWB file, the sweep group of a recording. Omit when the file holds one thing. It is part of the link's "
...
_VALUE_RELATION_DESCRIPTION = (
    "What the conversion did to the *values*: IDENTICAL for a lossless transcode (an ABF to a Zarr, a dataset to an NWB file), TRANSFORMED for a filtered or resampled "
    "export, CATEGORIZED when the values became labels. Omit when unstated; the converter and its parameters belong to task provenance"
)
...
@kante.pydantic_input(
    ...
    description=(
        "One file this container was produced from -- the ABF or NWB file a converter read to write these arrays. Recorded as a link between bytes and "
