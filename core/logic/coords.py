...
**Vendored from mikro** (``mikro/core/logic/coords.py``), kept at the same path so the two
copies stay diffable. What was cut is everything about *drawing*: render axes, the
array-order/vertex-order permutation, dataset specs and phasor axes. A dataset is a signal
over time, not an image, so nothing here derives a screen axis. The arithmetic is
untouched, and the vocabulary ("voxel", "pyramid") is mikro's: read "voxel" as "sample".

Several comments here and in `core.logic.graph` cite "item N of the proposals doc". That
document is **`testing/MIKRO_BACKEND_PROPOSALS.md`, in the `testing` repository** -- the
running record of known gaps in mikro's copy of this layer and their shipped state.
...
#
# A pint dimensionality, exponents included, rather than mikro's bare dimension name: a
# FREQUENCY axis is ``[time] ** -1``, which "this dimension, to the first power" cannot say.
_UNIT_DIMENSION_BY_TYPE: dict[str, dict[str, int]] = {
    enums.AxisTypeChoices.TIME.value: {"[time]": 1},
    enums.AxisTypeChoices.FREQUENCY.value: {"[time]": -1},
    enums.AxisTypeChoices.SPACE.value: {"[length]": 1},
}


def _dimension_label(dimension: dict[str, int]) -> str:
    """Spell a dimensionality the way the error message reads: ``[time]``, ``1 / [time]``."""
    return " * ".join(name if power == 1 else f"1 / {name}" if power == -1 else f"{name} ** {power}" for name, power in dimension.items())
...
def assert_unit_matches_type(axis_name: str, axis_type: str, unit: str) -> None:
    ...
    if dict(dimensionality) != required:
        raise AxisUnitError(f"Axis '{axis_name}' is a {axis_type} axis, so its unit must measure {_dimension_label(required)}, but '{unit}' does not. Use 'a.u.' for a genuinely arbitrary unit.")
...
#: absent here (COORDINATE, DISPLACEMENT, INDEX, VALUE) are deliberately unnamed: they
...
_SPEC_BY_AXIS_TYPE: dict[str, enums.ArrayDatasetSpec] = {
    ...
    enums.AxisTypeChoices.FREQUENCY.value: enums.ArrayDatasetSpec.SPECTRAL,
}
...
def vectors_bbox(vectors: Sequence[Sequence[float]], *, padding: float = 0.5) -> tuple[list[float], list[float]]:
    ...
    ``padding`` is elektro's one change here, and its default is mikro's behaviour. Half a
    cell is right when a vertex *names a cell of a grid* -- a sample index. It is wrong when
    a vertex is a *point of a continuum*: an event at 12.5 s padded to [12.0 s, 13.0 s) would
    answer range queries half a second off, which in electrophysiology is an eternity. The
    caller says which (see :func:`core.logic.graph.vertex_padding`).
    """
    low, high = aabb(vectors)
    return [value - padding for value in low], [value + padding for value in high]
