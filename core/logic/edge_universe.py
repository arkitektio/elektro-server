...
**Vendored from mikro** (``mikro/core/logic/edge_universe.py``). Identifiers say ``dataset``
where mikro says ``dataset`` (``seed_datasets``, ``dataset_ids``, the ``"dataset"`` container
key); nothing else changed. Read "scene" as "experiment" and "layer" as "view".
...
def root_edges_of(space: "models.CoordinateSystem | None", *, organization=None, loaders: dict | None = None) -> list["models.Transformation"]:
    ...
    # The edges touching this space *and the frames chained into it* -- a segment's clock
    # laid onto a session's, laid into this world. mikro fetches the space's own alone; see
    # `core.logic.graph.frames_into` for why that loses every recording placed through its clock.
    frames = graph_logic.frames_into(space)
    query = models.Transformation.objects.filter(Q(output__in=frames) | Q(input__in=frames), parent__isnull=True)
