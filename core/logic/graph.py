...
services, and because the reasoning is about graphs, not about images.
...
def assert_field_is_dereferenceable(field: "models.CoordinateSystem") -> None:
    ...
    **Exactly one dataset may live in the field's system (rule R3), and this is ours, not
    ...
    one frame (residence allows it), and "the array whose values are the map" is then not a
    function of the system: two residents, two candidate lookups, and nothing on the edge to
    say which. A times dataset therefore lives alone. A downsampled level's own system holds
    that one array and nothing else, so it qualifies as it does in mikro.

    A lens does not count: it is a selection over a dataset and owns no array, so there is
    nothing to sample.

    Deliberately **not** a store check. A zarr store is attached after its dataset row exists,
    so "no store yet" is a read-time concern; refusing it here would make the order of two
    unrelated writes load-bearing.
    """
    residents = list(field.datasets.all()[:2])
    if len(residents) == 1:
        return
    if len(residents) > 1:
        raise ValueError(
            f"More than one dataset lives in coordinate system '{field.name}', so which one's values are the map is not something the edge can say. "
            "A dataset whose values are a map -- a times dataset -- lives alone in its system: give it its own, and name that as the field."
        )
...
_SELECTABLE_AXIS_TYPES: frozenset[str] = frozenset(
    {
        ...
        enums.AxisTypeChoices.FREQUENCY.value,
    }
)
...
def frames_into(space: "models.CoordinateSystem | None") -> set[int]:
    """The ids of the reference frames chained into ``space``, ``space`` itself included.

    **Elektro's one addition to the search, and not in mikro.** A *frame* is a space nothing
    lives in. mikro roots every search at one such space and fetches the edges touching *it*,
    plus the facts of whatever data registers into it directly. That is enough there: an image
    is registered into a world in one hop, and the one two-hop shape it knows -- pixels ->
    physical space -> world -- is found only when a layer already names the dataset.

    Here the chain of frames **is** the ordinary layout: a dataset's sample grid maps onto its
    segment's clock, the segment's clock onto the session's, the session's into an
    experiment's world. Every hop but the first is an edge *between two frames*, which
    belongs to no dataset's own facts and does not touch the world -- so mikro's fetch never
    sees it, and a recording laid out through its clock was invisible to ``inView`` and to
    every ``placeableIn`` picker, and unplaceable at all past two hops.

    So a search rooted at a space also owns the frames that lead into it. Walked
    **backwards only**, over edges whose *input* is itself a frame: an edge out of a shared
    clock into some other experiment's world is that world's business, and following it
    would drag everything laid out there into this one. One batched query per generation of
    frames (a chain is two or three deep), and a seen-set, so a cycle cannot hang a request.
    An UNMAPPABLE edge relates no points and chains nothing.
    """
    if space is None:
        return set()
    ...
    while frontier:
        upstream = set(
            models.Transformation.objects.filter(parent__isnull=True, output_id__in=frontier, input_id__isnull=False)
            .exclude(kind=enums.TransformKindChoices.UNMAPPABLE.value)
            .values_list("input_id", flat=True)
        ) - frames
        if not upstream:
            break
        keys = container_map(upstream)
        frontier = {system_id for system_id in upstream if keys[system_id][0] == "system"}
        frames |= frontier
    return frames
...
def _placement_universe(
    ...
    edges = list(
        ...
        .filter(
            ...
            # The world's own edges, and those of the frames chained into it (`frames_into`):
            # mikro fetches `Q(input=world) | Q(output=world)` here.
            | Q(input__in=frames)
            | Q(output__in=frames)
        )
        .distinct()
...
def container_q(keys: "Iterable[tuple]", *, field: str) -> Q:
    ...
    for container in CONTAINERS:
        ...
        # `root_field` names the column holding the root's id (`dataset_id`); the join goes through the relation (`dataset`).
        lookup = f"{field}__{container.related_name}__in" if container.root_field == "pk" else f"{field}__{container.related_name}__{container.root_field.removesuffix('_id')}__in"
        query |= Q(**{lookup: pks})
    return query
...
def residence_map(system_ids: "Iterable[int]") -> dict[int, int]:
    ...
    **The direction is the whole point.** Ownership asked *space -> data*, which was a local
    ...
    for container in _KEYING_ORDER:
        if container.key != "dataset":
            continue
        for system_id, dataset_id in container.model.objects.filter(coordinate_system_id__in=ids).order_by("-pk").values_list("coordinate_system_id", container.root_field):
...
def _placement_seeds(space: "models.CoordinateSystem") -> set[tuple]:
    ...
    registrations = list(
        # `output__in=frames_into(world)`, where mikro has `output=world`: a dataset sampled onto
        # a clock that is itself laid into this world is placed here as surely as one
        # registered directly.
        models.Transformation.objects.filter(parent__isnull=True, output__in=frames_into(world))
...
def placeable_system_ids_in(space: "models.CoordinateSystem", *, derived_only: bool = False, require_affine: bool = True) -> set[int]:
    ...
    **``require_affine=False`` is elektro's, and only `SpaceGraph` passes it.** mikro has no
    such flag, for the reason the next paragraph gives, and every *picker* here still gets the
    strict set. But `inView` is not a picker: it answers "what is in this space", and a spike
    train or a variable-step run reaches its clock across a FIELD. Seeded from the strict set
    they were simply absent from their own clock's `inView` -- while `createExperiment` would
    show them. Asked loosely they come back badged ``NON_AFFINE``, with their path and no box.

    **Strict, with no flag to loosen it** (mikro's words, and still true of the default). Every caller of this is a picker -- the
    ...
    edges = list(
        models.Transformation.objects.filter(parent__isnull=True)
        # `frames`, where mikro has the world alone: see `frames_into`.
        .filter(container_q(container_keys, field="input") | container_q(container_keys, field="output") | Q(input__in=frames) | Q(output__in=frames))
        .distinct()
        .select_related("input", "output")
        .prefetch_related("children", "input__axes", "output__axes")
    )
    ...
    adjacency = adjacency_of(edges, admit_scoped=True, require_affine=require_affine)
...
def vertex_padding(system: "models.CoordinateSystem") -> float:
    """How far a vertex drawn in ``system`` extends around its coordinates: half a cell, or nothing.

    **Elektro's addition; mikro pads by half a voxel always.** A drawing space is unit-less,
    so what its numbers *are* is decided by what it is drawn over -- the first edge out of it:

    * into a **sample grid** (unit-less axes): a vertex names a sample, and sample ``n`` covers
      ``[n - 0.5, n + 0.5)``. Half a cell, as in mikro.
    * into a **clock or a world** (unit-carrying axes): a vertex is an instant, and an instant
      has no width. Padding it would make an event at 12.5 s overlap a query for 12.9 s.
    * into nothing: a freestanding space has no grid to have cells of. No padding.

    Read once per write and applied to the box that is stored; changing what a collection is
    drawn over afterwards does not re-pad boxes already written, exactly as it does not
    re-derive them (``created_with_transforms`` is how a client sees that).
    """
    edge = (
        models.Transformation.objects.filter(input=system, parent__isnull=True, output__isnull=False)
        .exclude(kind=enums.TransformKindChoices.UNMAPPABLE.value)
        .select_related("output")
        .prefetch_related("output__axes")
        .order_by("pk")
    ...
    if edge is None:
        return 0.0
    return 0.0 if any(axis.unit for axis in edge.output.axes.all()) else 0.5


def bbox_along_chain(chain: list, vectors: list[list[float]], *, padding: float = 0.5) -> dict | None:
    ...
    if not vectors:
        return None
    low, high = coords_logic.vectors_bbox(vectors, padding=padding)
    return coords_logic.transformed_bbox(low, high, chain)


def compute_intrinsic_bbox(system: "models.CoordinateSystem", vectors: list[list[float]], *, padding: float = 0.5) -> dict | None:
    """The bounding box of a shape's geometry, pushed into the nearest intrinsic space."""
    return bbox_along_chain(intrinsic_chain(system), vectors, padding=padding)
...
def layer_source_system(layer: "models.ExperimentLayer") -> "models.CoordinateSystem | None":
    ...
    mikro's function, over elektro's kinds. A trace, heatmap or waveform layer's data lives in its
    lens' space; a spikes layer's in the space its sparse dataset owns (a raster's ``(unit, t)``);
    an events, series or point layer's in the space its table owns (its TIME or SPACE coordinate
    columns); an annotation layer's in its collection's drawing space. Read the groups off
    :data:`core.enums.LENS_BACKED_KINDS` and :data:`core.enums.TABLE_BACKED_KINDS` rather than
    spelling them again.
    """
    ...
    if layer.kind in enums.TABLE_BACKED_KINDS and layer.table_dataset_id:
