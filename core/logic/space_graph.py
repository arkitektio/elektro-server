...
**Vendored from mikro** (``mikro/core/logic/space_graph.py``). What differs: residents are
org-scoped through :func:`core.scoping.organization_path`, because a lens and a level carry
no organization of their own and mikro's ``hasattr(model, "organization")`` lets them
through unscoped; candidates are gathered with ``require_affine=False`` and the forms are
built inside the ``try``, so a source timed by a lookup is reported ``NON_AFFINE`` rather
than left out; and a lens' shape is cut from the batched level-0 shapes rather than queried
per lens.
...
from core import enums, models, scoping
...
class Source:
    ...
    #: collection: a collection is its own container and searches from itself.
...
#: What each kind of resident needs fetched beside it for its extent to be read without a query
#: per row. A lens' shape is cut out of its dataset's, so the dataset and its axes' system ride
#: along (the level-0 shape itself comes from `SpaceGraph.shapes`, batched). A container absent
#: from this table needs nothing more: a level carries its own shape, and an annotation
#: collection is a set of shapes, not a box -- `_resident_box` answers None for it.
_SHAPE_RELATIONS: dict[type, tuple[str, ...]] = {
    models.Lens: ("dataset__coordinate_system",),
}
...
def _resident_box(resident: object, shapes: dict[int, list[int]]) -> tuple[list[float], list[float]] | None:
    ...
    if isinstance(resident, models.Lens):
        # Cut from the batched level-0 shape when it is in hand; `Lens.shape_list` asks the
        # database for that shape once per lens.
        base = shapes.get(resident.dataset_id)
        shape = coords_logic.lens_shape(base, resident.dataset.axis_names, resident.slices_list) if base else resident.shape_list
...
class SpaceGraph:
    ...
    def __init__(self, space: "models.CoordinateSystem", *, organization, loaders: dict | None = None) -> None:
        ...
        # Loosely: everything that *reaches* this space, a FIELD route included. `inView` reports
        # such a source as NON_AFFINE rather than leaving it out -- see `placeable_system_ids_in`.
        self._candidate_ids = graph_logic.placeable_system_ids_in(space, require_affine=False)
        ...
        if self._candidate_ids:
            for container in graph_logic.CONTAINERS:
                query = container.model.objects.filter(coordinate_system_id__in=self._candidate_ids, **{scoping.organization_path(container.model): organization})
                self._residents.extend(query.select_related("coordinate_system", *_SHAPE_RELATIONS.get(container.model, ())).prefetch_related("coordinate_system__axes"))
    ...
    def shapes(self) -> dict[int, list[int]]:
        ...
        Batched rather than read off ``ArrayDataset.shape_list``, which is a query per dataset and
        would make the cost of this whole graph grow with its source count.
        """
    ...
    def placement(self, source: Source, region: dict[str, list[float]], *, at: dict[str, int] | None = None) -> Hit | None:
        ...
        # Building the steps is inside the `try` as well as pushing the box through them: a
        # FIELD step has no matrix to build a form *from*. mikro builds them outside, which was
        # unreachable there -- its candidates are strictly affine -- and is the ordinary case
        # here, where a lookup-timed source is a candidate (see `placeable_system_ids_in`).
        try:
