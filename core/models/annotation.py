"""Annotations on datasets, and the collection that owns the space they are drawn in.

**Vendored from mikro** (``mikro/core/models/annotation.py``). It replaces this service's
``ROI``, which hung off one dataset by a foreign key and kept its extent in two integer
columns of sample indices -- so an event could mark one dataset and nothing else, in one unit
and no other.

An :class:`Annotation` is an addressable, mutable, owned shape a person (or a detector)
draws and edits: Neo's Event and Epoch, and the measurements drawn beside them. Annotations
never stand alone -- each belongs to an :class:`AnnotationCollection`, and the collection
owns its own coordinate system. How that drawing space relates to anything else is a
:class:`~core.models.Transformation` edge, never a second FK on the shape, and that is the
whole point of the move:

* drawn over a **dataset's sample grid**, a collection marks that dataset, in samples;
* drawn on a **segment's clock**, it marks *every signal of the segment at once*, in
  seconds -- which is what Neo's events and epochs are: they belong to the segment;
* drawn on an **experiment's world**, it marks the timeline, and every recording laid out
  on it, whatever clock each was recorded against.

Read "scene" as "experiment" below; what differs from mikro is that ``scene`` is
``experiment``, and the kinds are electrophysiology's
(:class:`core.enums.AnnotationKindChoices`).
"""
...
class AnnotationCollection(models.Model):
    ...
    ``experiment`` marks a collection minted as that experiment's default drawing surface
    by ``createAnnotation(experiment:)`` -- one per experiment, enforced by the database.
    ...
    # Filing, not placement -- see the note on `ArrayDataset.folder`. Distinct from `experiment`
    # below, which is also bookkeeping but answers a different question: `experiment` says which
    ...
    experiment = models.OneToOneField(
        ...
        help_text="The experiment this collection was minted for as its default drawing surface, if any. One per experiment; bookkeeping only, placement is the registration edge",
    )
    ...
    organization = models.ForeignKey(Organization, on_delete=models.CASCADE, related_name="annotation_collections", help_text="The organization this annotation collection belongs to")
...
class Annotation(models.Model):
    ...
    kind = TextChoicesField(
        ...
        default=enums.AnnotationKindChoices.EVENT.value,
        help_text="The geometric kind of the annotation (event, epoch, line, ...), which fixes how its vectors are read",
    )
    ...
    coordinates = models.JSONField(
        help_text="The discrete coordinates this annotation is pinned to, keyed by coordinate name, e.g. {'c': 3} for an event on channel 3, or {'sweep': 12}. A coordinate the annotation does not pin is one it spans",
        default=dict,
    )
