...
**Vendored from mikro** (``mikro/core/models/coords.py``), model code unchanged, so the
two services keep one ontology and the files stay diffable. The text below is mikro's and
speaks its vocabulary. Read it through this table:

=====================  ==========================================================
mikro says             elektro means
=====================  ==========================================================
dataset, ArrayDataset  the same: an :class:`~core.models.ArrayDataset` -- a recording
pixel grid, intrinsic  a dataset's **sample grid**: axes with types, never units
physical space         a **clock**: one TIME axis carrying a time unit
pixel size + offset    the **sampling law**: one edge, period and start time
scene                  an :class:`~core.models.Experiment`, over its ``world``
layer                  an experiment recording / stimulus view
pyramid level          the same: a decimated :class:`~core.models.DataArray`
registration           an offset or a clock synchronisation
=====================  ==========================================================

So where an image has a pixel size, a recording has a sampling rate; where a stage has a
position, a recording has a start time; and both are *edges*, never columns. Three things
are specific to this service and are stated where they apply: a spike train or an
irregularly sampled signal reaches time through a ``FIELD`` whose values are the sample
times; the unit of a dataset's *values* (mV, pA) is a ``ValueUnit`` anchor and never an
axis; and a system holding a times dataset holds nothing else (see
:func:`core.logic.graph.assert_field_is_dereferenceable`). ``core/DESIGN.md`` has the
full mapping and every place this port deliberately differs from mikro.
...
class CoordinateSystem(models.Model):
    ...
    ``DataArray.coordinate_system``, ``Lens.coordinate_system`` and
    ``AnnotationCollection.coordinate_system`` (mikro has four more).
