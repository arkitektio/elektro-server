"""Filing: folders, the raw files in them, and the links between a file's bytes and the data it encodes.

**Vendored from mikro** (``mikro/core/models/folder.py``). It replaces this service's ``Dataset``
-- the same model under a name that collided with the data it filed -- and its ``File``. A
file link's container is one of two here (an array dataset, an annotation collection) where
mikro has four; the table and mesh arms are gone with those models.
"""
