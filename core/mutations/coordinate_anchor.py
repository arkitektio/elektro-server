...
converter that knows the rig state as it writes the samples. It does not serve the task
that computes a histogram a week later, or the user who labels a channel of a table
...
class CreateCoordinateAnchorInput:
    ...
    anchor: CoordinateAnchorInput = strawberry.field(
        description=(
            ...
            "axis-typed columns and values along them; for a sparse dataset they name its enumerated axes and positions along them. An empty `axisAnchors` list is an anchor global over the whole container. A value unit is array-only"
        )
    )
...
def create_coordinate_anchor(info: Info, input: CreateCoordinateAnchorInput) -> types.CoordinateAnchor:
    ...
    with transaction.atomic():
        if model.dataset is not None:
            ...
            assert_anchors_name_axes([model.anchor], [spec.name for spec in container.axis_specs], what="dataset")
        ...
        _write_anchor_spokes(info, anchor, model.anchor)
