...
**Vendored from mikro** (``mikro/core/mutations/coordinate_system.py``). Sources are a
dataset, a lens or a coordinate system; "is some composition laid out over this space" reads
:data:`core.logic.graph.WORLD_RELATIONS` where mikro spells ``scenes``; and deletes go
through :func:`core.guards.enforce_delete`, this service's one delete predicate, where
mikro has its own ``assert_can_delete``. Read "scene" as "experiment".
...
def create_coordinate_system(info: Info, input: CreateCoordinateSystemInput) -> types.CoordinateSystem:
    ...
    for spec in model.registrations:
        source_system = coordinate_system_logic.resolve_source_system(
            ...
            lens=get_for_org(models.Lens, info, id=spec.lens) if spec.lens else None,
...
def delete_coordinate_system(info: Info, input: DeleteCoordinateSystemInput) -> strawberry.ID:
    ...
    for relation in graph_logic.WORLD_RELATIONS:
        ...
        if composed:
            ...
            raise ValueError(
                f"Coordinate system {system.pk} is the space {len(composed)} {noun}(s) are laid out in ({', '.join(str(row.pk) for row in composed)}) and cannot be deleted. "
                "A shared space outlives whatever adopts it; delete those first."
            )
    ...
    guards.enforce_delete(info, system)
    system.delete()
    return model.id
...
def delete_orphaned_coordinate_systems(info: Info) -> list[strawberry.ID]:
    ...
    orphans = (
        ...
        .filter(**graph_logic._UNINHABITED, **{f"{relation}__isnull": True for relation in graph_logic.WORLD_RELATIONS})
        # Not an orphan while something still *names* it under PROTECT: a time lookup's field (the
        # times dataset was deleted, the edge reading through its system was not), or the frame an
        # annotation collection's boxes are denominated in. Neither is an endpoint of an edge, so
        # `touched` below does not see them -- and one such row would fail the whole sweep.
        .filter(fields_of__isnull=True, annotation_bbox_frames__isnull=True)
        .exclude(Exists(touched))
    )
    if not guards.is_org_admin(info):
