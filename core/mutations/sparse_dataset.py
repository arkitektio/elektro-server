"""Creating a sparse dataset: identified axes, and one or more stored layouts.

**Vendored from mikro** (``mikro/core/mutations/sparse_dataset.py``). elektro's changes, each
marked where it happens: an axis may be TIME (a spike raster's samples) and is then identified
by nothing; axis types reach ``split_identifications``; the delete sweeps the owned space and
takes no ``owner`` (``core/guards.py`` is the one predicate).
...
from core.mutations._generic import make_owned_space_delete
...
def _resolve_identifications(
    ...
    elektro: the axis types are passed for real rather than as ``None``, because a spike
    raster has a TIME axis, and a table identifying one would claim its sample positions are
    row ids -- the same refusal a table's SPACE column gets. A TIME axis is identified by
    nothing at all (a sampling law places it), so it is the one axis allowed an empty list, and
    the one refused a non-empty one.

    What is left is the two things a shape cannot state: that an INDEX axis is identified at
    all, and that *something* keys this matrix.
    """
    timed = [axis.name for axis in model.axes if axis.type == enums.AxisType.TIME and axis.identified_by]
    if timed:
        raise ValueError(
            f"'{model.name}' identifies its TIME axis {timed}, but a TIME axis' positions are samples, not ids: nothing's contents are them and no table's rows are. "
            "Leave `identifiedBy` empty and place the axis on a clock with a sampling law (`createSamplingLaw`) once the dataset exists."
        )
    empty = [axis.name for axis in model.axes if axis.type == enums.AxisType.INDEX and not axis.identified_by]
    if empty:
    ...
    references, keyed = identification_logic.split_identifications(
        ...
        axis_types={axis.name: axis.type for axis in model.axes},
    )
...
def create_sparse_dataset(info: Info, input: CreateSparseDatasetInput) -> types.SparseDataset:
    ...
    # elektro: `SparseAxisInput` has a `type` here (mikro's has none), because a spike raster's
    # sample axis is TIME. Nothing else is admitted -- a sparse matrix has no spatial or channel
    # axes -- and at most one TIME axis, because one sampling law places a raster.
    wrong = sorted({axis.name for axis in model.axes if axis.type not in (enums.AxisType.INDEX, enums.AxisType.TIME)})
    if wrong:
        raise ValueError(f"'{model.name}' declares {wrong} with a type other than INDEX or TIME. A sparse matrix enumerates (INDEX) and may have one sample axis (TIME); nothing else.")
    if sum(axis.type == enums.AxisType.TIME for axis in model.axes) > 1:
        raise ValueError(f"'{model.name}' declares more than one TIME axis. A spike raster has one sample axis, and one sampling law places it; a second would be a second clock.")
    if all(axis.type == enums.AxisType.TIME for axis in model.axes):
        raise ValueError(f"'{model.name}' has no INDEX axis. A sparse matrix enumerates something -- units, channels, genes -- along at least one axis.")
    ...
    with transaction.atomic():
        ...
        # elektro: the declared type (INDEX or TIME) rather than a supplied INDEX. Still pixel
        # axes -- no unit -- because a sample axis is "TIME with no unit" (DESIGN.md): physical
        # time enters through the sampling law, never through the grid.
        graph_logic.create_pixel_axes(
            system,
            [
                AxisInputModel(name=axis.name, type=axis.type, long_name=axis.long_name, description=axis.description)
        ...
        for anchor_input in model.anchors or []:
            ...
            _write_anchor_spokes(info, anchor, anchor_input)
...
delete_sparse_dataset = make_owned_space_delete(models.SparseDataset, DeleteSparseDatasetInput, guard=pickers.assert_sparse_dataset_not_in_a_picker)
