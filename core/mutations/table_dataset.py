...
**Vendored from mikro** (``mikro/core/mutations/table_dataset.py``). In elektro a table is
also what a spike raster's units are (one row per sorted unit, keyed by its INDEX column) and
what events and epochs are (a TIME coordinate column, an optional stop column). elektro's
changes: no network-node identification, and the delete sweeps the owned space and takes no
``owner``.
"""
...
class ColumnInput:
    ...
    identified_by: list[IdentificationInput] = strawberry.field(
        ...
        description=(
            ...
            "`references` field). On an INDEX axis: a DATASET authors a FIELD edge from that source into this table -- the direction "
            ...
            "ATTRIBUTE/ID column referencing a table of units: the edge of the join graph `colorBys` walks). Refused on SPACE/TIME axes, whose values are positions and cannot also be ids"
        ),
    )
...
class CreateTableDatasetInput:
    ...
    anchors: list[CoordinateAnchorInput] | None = strawberry.field(
        default=None,
        description=(
            "Optional coordinate anchors pinning metadata spokes -- the rig state, acquisition metadata, a value histogram, a channel label, a recording or stimulus site, the "
            "simulation state -- to values of the table's coordinate columns: each `axisAnchors` entry names an axis-typed column and the (integer) value along it, and an empty "
            "`axisAnchors` list is an anchor global over the whole table. The same hub an array dataset uses, so a per-unit table carries the acquisition facts of the recording it "
            "was computed from. A value unit is array-only: a table's units are its columns'"
        ),
    )
...
def create_table_dataset(info: Info, input: CreateTableDatasetInput) -> types.TableDataset:
    ...
    # The identifications, split into the two things they become: the tables a column
    # enumerates or references, and the sources that author a FIELD edge -- one entry per source, so an axis keyed by
    ...
    # a data column.
    reference_targets, keyed = identification_logic.split_identifications(
        info,
        name=model.name,
...
delete_table_dataset = make_owned_space_delete(models.TableDataset, DeleteTableDatasetInput, guard=pickers.assert_table_not_in_a_picker)
