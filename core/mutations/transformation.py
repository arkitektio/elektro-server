...
**Vendored from mikro** (``mikro/core/mutations/transformation.py``). Create and update are
unchanged. Deletes go through :func:`core.guards.enforce_delete`; mikro's picker guard is
gone with the scene pickers it protected; and ``deleteRegistration`` names a dataset, a lens
or a coordinate system as its source.
...
import kante
from core import enums, guards, models, types
...
def delete_transformation(info: Info, input: DeleteTransformationInput) -> strawberry.ID:
    """Delete one edge by id. A wrapper takes its children with it (``parent`` is CASCADE).

    Org-scoped like every other id a client sends, then held to this service's one delete
    rule (:func:`core.guards.can_delete`): an org admin, the assigner of the task that wrote
    the edge, or its creator. That makes a server-derived edge -- a sampling law written by
    ``createBlock``, a lens' shift -- deletable by whoever created the data it describes,
    which is deliberate: it is how a wrong sampling rate gets replaced.
    """
    model = input.to_pydantic()
    edge = get_for_org(models.Transformation, info, id=model.id)
    guards.enforce_delete(info, edge)
    edge.delete()
...
class DeleteRegistrationInput:
    ...
    lens: strawberry.ID | None = strawberry.field(default=None, description="Un-register this lens. Provide exactly one source")
...
def delete_registration(info: Info, input: DeleteRegistrationInput) -> list[strawberry.ID]:
    ...
    source_system = coordinate_system_logic.resolve_source_system(
        ...
        lens=get_for_org(models.Lens, info, id=model.lens) if model.lens else None,
