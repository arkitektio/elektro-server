...
from core.types._shared import OrgScoped, OrgScopedOrNested
...
# What can live in a space, as one field. The arms mirror `core.logic.graph.CONTAINERS`,
# which is the list; adding a member there without adding it here is how a resident becomes
# unreturnable. The array types live in a module that imports this one, so they are
# annotated lazily.
Resident = Annotated[
    Union[
...
class CoordinateSystem(OrgScoped):
    ...
    @kante.django_field(
        description=(
            "The annotations drawn in a space that can reach this one. Reachability, not containment: asked of a segment's clock it answers with the events and epochs drawn on the "
            "clock *and* those drawn over any dataset sampled onto it; asked of an experiment's world, with everything marked on anything laid out there. An annotation belongs to a "
            "collection, and outlives every experiment composing over this space"
        ),
    )
    def annotations(self, info: Info) -> List[Annotated["Annotation", strawberry.lazy("core.types.annotation")]]:
        ...
        return list(
            models.Annotation.objects.filter(Q(collection__coordinate_system__in=placeable) | Q(collection__coordinate_system__in=anchored), collection__organization=info.context.request.organization)
    ...
    def in_view(self, info: Info, region: BoundingBoxInput, at: List[CoordinateInput] | None = None) -> List["SourcePlacement"]:
        ...
        # Anchors are thousands of rows on a long recording, so they are resolved only when
...
InViewSource = Annotated[
    ...
    strawberry.union(
        "InViewSource",
        description=(
            "A container registered into a coordinate system: the data that can be in view of a region asked in it. A lens appears only where its dataset does not -- a window "
            "registered into a world its recording has no route to -- because reporting both would return the same samples twice"
        ),
    ),
]
