...
**Vendored from mikro** (``mikro/core/types/sparse_dataset.py``). elektro: every type mixes in
tenant scoping, as every type here does, and the dataset's ``sourceFiles`` reads links mikro
cannot record (a sparse dataset is a ``FileLink`` container here).
"""
...
class SparseArray(OrgScopedOrNested):
...
class SparseAxisReference(OrgScopedOrNested):
