> **Vendored from mikro** (`mikro/docs/rfc10-affine-placement-gate.md`), unchanged below this note, because the vendored code in `core/logic`, `core/models/coords.py` and `core/types/coords.py` cites it by number.
> It speaks mikro's vocabulary -- datasets, scenes, layers, pixels. For how each of those reads in electrophysiology, and for every place this service differs, see [`core/DESIGN.md`](../core/DESIGN.md).
>
> **elektro deliberately does not apply this gate to experiment views.** A spike train, an irregularly sampled signal and a variable-step simulation reach time across a FIELD, which never condenses, and a timeline can draw them without a matrix. The `placeableIn` *filters* stay strict, as here. See `core/DESIGN.md`, "Where this port differs from mikro".
