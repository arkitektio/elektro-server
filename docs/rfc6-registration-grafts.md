> **Vendored from mikro** (`mikro/docs/rfc6-registration-grafts.md`), unchanged below this note, because the vendored code in `core/logic`, `core/models/coords.py` and `core/types/coords.py` cites it by number.
...
>
> Historical in mikro too: most of it was reversed by RFC-9 (see its own amendments). Kept because the vendored code still cites it for the *fact tree* and the primary-parent rule, both of which survive.
