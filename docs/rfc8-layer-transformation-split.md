> **Vendored from mikro** (`mikro/docs/rfc8-layer-transformation-split.md`), unchanged below this note, because the vendored code in `core/logic`, `core/models/coords.py` and `core/types/coords.py` cites it by number.
...
>
> The rule elektro's experiment views are held to: a view carries no `offset` and no `duration`. Read "layer" as an experiment's recording or stimulus view, and "scene" as an experiment.
