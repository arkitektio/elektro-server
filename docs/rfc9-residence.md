> **Vendored from mikro** (`mikro/docs/rfc9-residence.md`), unchanged below this note, because the vendored code in `core/logic`, `core/models/coords.py` and `core/types/coords.py` cites it by number.
...
>
> The current model, and the one elektro implements. Read "dataset" as a trace, "pixel grid" as its sample grid, "physical space" as a clock. elektro's containers are a trace and a lens; mikro's are eight.
